...
<main class="content-wrapper">
    <div class="container-fluid py-3">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>{{ isset($bangleBoxSize) ? 'Edit Box Size' : 'Add New Box Size' }}</h2>
            <a href="{{ route('admin.box-sizes') }}" class="btn text-white" style="background-color: #6cc2b6; border-color: #6cc2b6;">← Back</a>
        </div>
...
@csrf
   ...
   <div class="mb-3">
    ...
    <input type="number" step="0.01" class="form-control" id="price" name="price"
           value="{{ old('price', $bangleBoxSize->price ?? '') }}" required>
</div>


    <!-- Submit -->
    <button type="submit" class="btn" style="background-color:#6cc2b6;color:white;">
        {{ isset($bangleBoxSize) ? 'Update Box Size' : 'Save Box Size' }}
    </button>
</form>
...
$(document).ready(function () {
    $('#bangleBoxSizeForm').on('submit', function (e) {
        ...
        $.ajax({
            url: "{{ route('admin.box-size.store') }}", // single route
            method: "POST",
            data: formData,
             success: function(response) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: response.message || 'Box Size added successfully!',
                }).then(() => {
