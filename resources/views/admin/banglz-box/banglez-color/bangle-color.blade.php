...
<main class="content-wrapper">
    ...
    <div class="container-fluid py-3">
        <div class="heading-top d-flex justify-content-between align-items-center">
            <h1 class="mb-0">All Bangle Color</h1>
            <a href="{{ route('admin.bangle-color.create') }}" class="btn text-white " style="background-color: #6cc2b6; border-color: #6cc2b6;">+ Add Bangle Color</a>
        </div>
    </div>
    <div class="client-table pt-2">
        <div class="row g-2 align-items-center mb-3">
            ...
            <div class="col-11">
                <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Search Bangle Color...">
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        var table = $('#box-size-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('admin.get-bangle-color-list') }}", // using same route
                type: 'GET',
                data: function(d) {
            ...
            language: {
                ...
                emptyTable: "No Bangle Color found",
                zeroRecords: "No Bangle Color found"
            }
        });
        var searchTimer;
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "This Bangle Color will be deleted!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                // Use route URL
                const url = "{{ route('admin.bangle-color.delete', ':id') }}".replace(':id', id);
